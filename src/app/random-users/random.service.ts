...
export class RandomService {
  ...
  getData(): Observable<RandomUsersResponse> {
    const x = this.http.get<RandomUsersResponse>(
  ...
  getResults(): Observable<Result[]> {
    return this.http
      .get<RandomUsersResponse>("https://randomuser.me/api?results=10")
  ...
  getGenderPeople(n: number, gender: string): Observable<Result[]> {
    return this.http
      .get<RandomUsersResponse>("https://randomuser.me/api?results=" + n)
      .pipe(
        map((response) => response.results.filter((x) => x.gender === gender))
      );
  }
